def RuteadorCompleto(inputLocations, InputDemoras, InputNum_vehicles, InputCarga_max, InputDemandas = 1, printSolution = False, plotSolution = False):
    ...
    def create_data_model(locations, demoras, num_vehicles, carga_max, demandas = 1):
        """Stores the data for the problem."""
        data = {}
        data['locations'] = locations
        data['num_vehicles'] = num_vehicles
        data['depot'] = 0
        data['demoras'] = demoras
    ...
    def main(locations, demoras, num_vehicles, carga_max, demandas, printSolution, plotSolution):
        """Solve the VRP with time windows."""
        # Instantiate the data problem.
        data = create_data_model(locations, demoras, num_vehicles, carga_max, demandas)
        ...
        for node in range(1, len(distance_matrix)):
            # calculate penalty based on time window
            # penalty = 10000 * (data['demoras'][node - 1] + 1)  # add 1 to avoid division by zero
            penalty =  max(dist for inner_dict in distance_matrix.values() for dist in inner_dict.values() if dist != 0) * data['demoras'][node - 1]
    ...
    routes, distances  = main(inputLocations, InputDemoras, InputNum_vehicles, InputCarga_max, InputDemandas, printSolution, plotSolution)
